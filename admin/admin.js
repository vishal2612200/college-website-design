...
document.querySelector('#sub').addEventListener('click', function () {
  ...
  function updateDatabase(fileName,descrip,category)
  {
    ...
    archive.readFile('/notices/data.json','utf8').then(function(result){
     date =  new Date().toJSON().slice(0,10).replace(/-/g,'/');
      ...
      var obj = {id : `${id}`, date : `${date}` ,path : `/notices/${fileName}`, description : `${descrip}`,category : `${category}`}
          console.log(k)

      //k.push(obj)
      k.push(obj)
    archive.writeFile('/notices/data.json',JSON.stringify(k)) 
    console.log(k)
  ...
  function indexing(description){
    ...
    archive.readFile('/index/notices.json','utf8').then(function(result){
      read=JSON.parse(result)
      id = 0
      var obj = {id : `${id}`,tags : `${tags}`}
      read.push(obj)
          archive.writeFile('/index/notices.json',JSON.stringify(read))
      return id
        })
